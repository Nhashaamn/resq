...
class NearbySafeZonesState {
  final List<NearbySafeZone> nearbyZones;
  final bool isLoading;
  final String? error;
  final Position? userLocation;

  NearbySafeZonesState({
  ...
  NearbySafeZonesState copyWith({
    List<NearbySafeZone>? nearbyZones,
    bool? isLoading,
    String? error,
    Position? userLocation,
  }) {
    return NearbySafeZonesState(
  ...
  NearbySafeZone? get nearestZone {
...
class NearbySafeZonesNotifier extends StateNotifier<NearbySafeZonesState> {
  final SafeZoneRepository _zoneRepository;
  StreamSubscription? _zonesSubscription;
  StreamSubscription<Position>? _positionSubscription;
  List<SafeZone> _allZones = [];
  static const double _radiusKm = 50.0; // 50km radius

  NearbySafeZonesNotifier(this._zoneRepository) : super(NearbySafeZonesState()) {
    _initialize();
  }
  ...
  void _checkNearbyZones(Position? userLocation) {
    ...
    final nearbyZones = <NearbySafeZone>[];
    ...
    for (var zone in _allZones) {
      ...
      if (zone.type == SafeZoneType.circle && zone.center != null && zone.radius != null) {
        ...
        if (distanceInKm <= _radiusKm) {
          nearbyZones.add(NearbySafeZone(
            zone: zone,
            distanceInKm: distanceInKm,
            createdAt: zone.createdAt,
          ));
        }
      } else if (zone.type == SafeZoneType.polygon && zone.polygonPoints != null && zone.polygonPoints!.isNotEmpty) {
...
final nearbySafeZonesProvider = StateNotifierProvider<NearbySafeZonesNotifier, NearbySafeZonesState>((ref) {
  final zoneRepository = getIt<SafeZoneRepository>();
  return NearbySafeZonesNotifier(zoneRepository);
});
